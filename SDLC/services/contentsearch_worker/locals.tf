locals {
  container_image = "${var.image_repository}:${var.image_commit_hash}"
  # container_port = 8080
  container_port = 80
  ...
  task_min_count                      = var.dark_enabled ? 0 : (var.service_name == "worker" ? 1 : var.task_min_count_config_map[terraform.workspace])
  task_max_count                      = var.dark_enabled ? 1 : (var.service_name == "worker" ? 1 : var.task_max_count_config_map[terraform.workspace])
